...
async function verifyForgotPasswordRecoverCode(req, res, next) {
  try {
    ...
    if (
      Date.now() <= foundUser.timestamp &&
      code === foundUser.password_recover_code
    ) {
      await UserModel.findByIdAndUpdate(foundUser._id, {
        password_recover_code: true,
      });
      logger.info(
        `${email} email egasi parolni tiklash kodini tasdiqlashdan o'tkazdi!`
      );
      return res
        .status(200)
        .json({ message: "Parolni tiklash kodi tasdiqlandi!" });
    }
  ...
  try {
    ...
    if (Date.now() <= foundUser.allowed_time && foundUser.attempts >= 3) {
      const waitTime = Math.ceil(
        (foundUser.allowed_time - Date.now()) / 1000 / 60
      );
      return next(
        new BaseError(
          429,
...
async function getUsers(req, res, next) {
  try {
    ...
    if (users.length === 0) {
      return next(
        BaseError.BadRequest(404, "Hali foydalanuvchilar mavjud emas!")
      );
    }
...
async function show_user_data(req, res, next) {
  try {
